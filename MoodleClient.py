...
class MoodleClient(object):
    ...
    def upload_file(self,file,saved = True):
        ...
        data['userdata'] = self.getUserData()        
        ffname = str(str(data['url']).split('/')[-1]).replace('?forcedownload=1','')
        data['directurl'] = self.path+'webservice/pluginfile.php/'+data['ctxid']+'/user/private/'+ffname+'?token='+data['userdata']['token']
